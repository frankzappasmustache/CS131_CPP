...
class Invoice {
 public:
  // Invoice(string mpartNumber, string mpartDescription, int mitmQuantity, int
  // mitmPrice)
  /*{
     partNumber{mpartNumber};
     partDescription{mpartDescription};
     itmQuantity{mitmQuantity};
     itmPrice{mitmPrice}string;
  };
  //*/
  void setPartNumber(std::string mpartNumber) { partNumber = mpartNumber; }

  void setPartDescription(std::string mpartDescription) {
    partDescription = mpartDescription;
  }

  void setItmQuantity(int mitmQuantity) { itmQuantity = mitmQuantity; }

  void setItmPrice(int mitmPrice) { itmPrice = mitmPrice; }

  std::string getPartNumber() { return partNumber; }
  std::string getPartDescription() { return partDescription; }
  int getItmQuantity() { return control(itmQuantity); }
  int getItmPrice() { return control(itmPrice); }
  int getInvoiceAmount() { return control(itmQuantity * itmPrice); }
  int control(int x) {
    if (x < 0) {
      return 0;
    } else {
